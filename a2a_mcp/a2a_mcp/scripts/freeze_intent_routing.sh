...
CAPSULE_PATH="${ROOT_DIR}/capsules/doctrine/capsule.gpt.intent.routing.v1.json"
CANON_SCRIPT="${ROOT_DIR}/scripts/canonicalize_manifest.py"
OUTPUT_DIR="${ROOT_DIR}/runtime"
CANONICAL_PATH="${OUTPUT_DIR}/capsule.gpt.intent.routing.v1.canonical.json"
HASH_PATH="${OUTPUT_DIR}/capsule.gpt.intent.routing.v1.sha256"
LEDGER_PATH="${OUTPUT_DIR}/scrollstream_ledger.jsonl"

if [[ ! -f "${CAPSULE_PATH}" ]]; then
  echo "Routing policy capsule not found at ${CAPSULE_PATH}" >&2
  exit 1
fi
...
entry = {
    "timestamp": timestamp,
    "event": "intent-routing.freeze",
    "capsule": "capsule.gpt.intent.routing.v1",
...
printf "Canonical routing policy written to %s\n" "${CANONICAL_PATH}"
