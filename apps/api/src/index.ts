...
async function getCurrentState() {
  ...
  if (!latest) {
    ...
    await prisma.ssotState.create({ data: { state: state as any, hash } });
    return { state, hash };
  }
...
app.post("/game/act", async (request, reply) => {
  ...
  await prisma.actionLog.create({ data: { actor: action.actor, action: action as any } });
  await prisma.ssotState.create({ data: { state: result.state as any, hash } });
