...
from ..services.pickle_rick_agent import PickleRickAgent
...
@router.post("/pickle-rick")
async def pickle_rick(request: WebsiteTemplateRequest) -> dict:
    """Pickle Rick agent: Always allow actions and maintain contextual coherence."""
    rick = PickleRickAgent()
    return rick.generate_implementation_plan(request)
