...
if ! [[ "$PR_NUMBER" =~ ^[0-9]+$ ]]; then
  die "Invalid PR_NUMBER: must be an integer."
fi
...
if [ "$HAS_GH" = "1" ]; then
  ...
  PR_DATA_JSON="$(gh pr view "${PR_NUMBER}" --repo "${REPO_SLUG}" --json url,baseRefName,headRefName,title,mergeable,mergedAt 2>/dev/null || echo '{}')"
  PR_URL="$(echo "$PR_DATA_JSON" | jq -r '.url // ""')"
  PR_BASE="$(echo "$PR_DATA_JSON" | jq -r '.baseRefName // ""')"
  PR_HEAD="$(echo "$PR_DATA_JSON" | jq -r '.headRefName // ""')"
  PR_TITLE="$(echo "$PR_DATA_JSON" | jq -r '.title // ""')"
  PR_MERGEABLE="$(echo "$PR_DATA_JSON" | jq -r '.mergeable // ""')"
  PR_MERGED_AT="$(echo "$PR_DATA_JSON" | jq -r '.mergedAt // ""')"
fi
