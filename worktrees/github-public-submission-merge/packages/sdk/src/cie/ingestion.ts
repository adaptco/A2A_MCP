...
import plugSchema from "../schemas/cie_v2_plug.v1.schema.json" with { type: "json" };
import invariantSchema from "../schemas/invariant_set.v1.schema.json" with { type: "json" };
import { CieV2PlugV1, InvariantSetV1, KernelTokenV1 } from "./types.js";

const ajv = new ( Ajv as any )( { allErrors: true, strict: true } );
ajv.addSchema( invariantSchema as any, "invariant_set.v1.schema.json" );
const validatePlug = ( ajv as any ).compile( plugSchema as any );
const validateInvariant = ( ajv as any ).compile( invariantSchema as any );
