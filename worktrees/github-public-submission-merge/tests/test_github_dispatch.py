...
from core_orchestrator.github_dispatch import DEFAULT_EVENT_TYPE, make_tx_id, send_block
...
def test_make_tx_id_deterministic():
    payload = {"foo": "bar", "baz": 123}
    tx1 = make_tx_id(payload, salt_with_uuid=False)
    tx2 = make_tx_id(payload, salt_with_uuid=False)
    assert tx1 == tx2
    assert len(tx1) == 64  # full sha256 hex digest


def test_make_tx_id_normalization():
    payload1 = {"a": 1, "b": 2}
    payload2 = {"b": 2, "a": 1}
    tx1 = make_tx_id(payload1, salt_with_uuid=False)
    tx2 = make_tx_id(payload2, salt_with_uuid=False)
    assert tx1 == tx2


def test_make_tx_id_salted():
    payload = {"foo": "bar"}
    tx1 = make_tx_id(payload, salt_with_uuid=True)
    tx2 = make_tx_id(payload, salt_with_uuid=True)

    assert tx1 != tx2
    assert tx1.startswith(tx2[:16])
    assert len(tx1) == 16 + 1 + 8  # prefix + '-' + short uuid


def test_make_tx_id_different_payloads():
    tx1 = make_tx_id({"a": 1}, salt_with_uuid=False)
    tx2 = make_tx_id({"a": 2}, salt_with_uuid=False)
    assert tx1 != tx2
