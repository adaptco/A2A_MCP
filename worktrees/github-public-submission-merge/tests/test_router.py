...
def test_router_rejects_unknown_pr_template_id():
    event = Event(
        source="tests",
        type="message.created",
        payload={"content": "hello", "pr_template_id": "PRT.UNKNOWN.V1"},
    )
    parser = SingleEventParser(event)
    sink = RecordingSink()
    router = Router([parser], [sink])
...
def test_router_allows_known_pr_template_id():
    event = Event(
        source="tests",
        type="message.created",
        payload={"content": "hello", "pr_template_id": "PRT.AUTO_REPAIR.V1"},
    )
    parser = SingleEventParser(event)
    sink = RecordingSink()
    router = Router([parser], [sink])
